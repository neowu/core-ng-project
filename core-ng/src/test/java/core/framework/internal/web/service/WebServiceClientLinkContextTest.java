...
class WebServiceClientLinkContextTest {
    ...
    void linkContext() {
        ...
        assertThat(request.headers).containsKeys(HTTPHandler.HEADER_CLIENT.toString(),
                ...
                HTTPHandler.HEADER_TIMEOUT.toString());
        assertThat(Long.parseLong(request.headers.get(HTTPHandler.HEADER_TIMEOUT.toString()))).isGreaterThan(0).isLessThanOrEqualTo(Duration.ofSeconds(1).toNanos());
